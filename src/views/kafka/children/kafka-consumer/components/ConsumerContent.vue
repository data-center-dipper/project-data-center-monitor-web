  ...
  <div class="topicConsumerShow">
    ...
    <div v-else-if="consumers.length > 0">
      ...
      <ConsumerCard
        ...
        :consumer="consumer"
      />
      <el-button class="mt-4" style="width: 100%" @click="onAddGroup()"
        >增加消费组</el-button
      >
    </div>
    <div v-else>
      <p v-if="selectedTopics || selectedConsumerGroups">
        请选择一个主题或消费组以查看相关内容。
      </p>
...
watch(
  () => [props.selectedTopics, props.selectedConsumerGroups],
  async ([newTopic, newGroup]) => {
...
function onAddGroup() {
  ...
  useDialog({
    title: '添加新的消费组',
    component: ConsumerGroupDetail, // 确保这是正确的组件引用
    props: {},
  })
}
</script>
