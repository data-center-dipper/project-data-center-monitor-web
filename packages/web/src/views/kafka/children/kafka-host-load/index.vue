<template>
  <div class="page-container" style="display: flex; flex-direction: column;">
    ...
    <section class="monitor-section" v-if="kafkaMonitors.length > 0">
      ...
      <hr />
      <div class="monitor-list" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 10px;">
        <div v-for="(monitor, index) in paginatedKafkaMonitors" :key="index" class="monitor-info" @click="showDetail(monitor)">
          <p>主机域名：<router-link to="/details">{{ monitor.hostDomain }}</router-link></p>
      ...
      <!-- 分页控件 -->
      <pagination v-if="totalMonitorPages > 1" :current-page="currentMonitorPage" :total-pages="totalMonitorPages" @page-change="handleMonitorPageChange" />
    </section>

    <!-- 显示主题和主机总数 -->
    <div class="info-section" style="border: 1px solid #ddd; width: 600px; margin: 20px auto; text-align: center; padding: 10px;">
      <span>主题: </span>
      <input type="text" v-model="searchQuery" placeholder="搜索..." style="margin-right: 10px;" />
      <span>主机总数: {{ kafkaMonitors.length }}</span>
    </div>

    <!-- 主机详情展示 -->
    <div class="selected-hosts-section" style="display: flex; flex-direction: column; gap: 10px;">
      <div v-for="host in selectedHosts" :key="host.hostDomain" class="host-details" style="border: 1px solid #ddd; padding: 10px; box-sizing: border-box;">
