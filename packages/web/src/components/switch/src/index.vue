  ...
  >
    ...
    <div class="star-cloud-box">
      <div class="star-box" :class="{ 'star-move': !ballStatus }">
        <div
          ...
          :key="star.id"
          class="star"
          :class="{ twinkle: ifTwinkle(star.id) }"
          :style="{
      ...
      <div class="cloud-box">
        <div class="cloud-near" :class="{ 'cloud-far-move': ballStatus }">
          <div
            v-for="(cloud, index) in cloudNearList"
            :key="index"
            class="cloud"
            :class="{ 'cloud-near-shake': shakeStatus }"
            :style="getCloudNearStyle(cloud)"
          ></div>
        </div>
        <div class="cloud-far" :class="{ 'cloud-far-move': ballStatus }">
          <div
            v-for="(cloud, index) in cloudFarList"
            :key="index"
            class="cloud"
            :class="{ 'cloud-far-shake': shakeStatus }"
            :style="getCloudFarStyle(cloud)"
          ></div>
        </div>
      </div>
    </div>
    <div class="halo-box">
      <div class="halo-linear" v-if="halo === 'linear'">
        <div
          class="halo-middle"
          :class="{ 'halo-left': !ballStatus, 'halo-right': ballStatus }"
        ></div>
      </div>
...
const dayHoverAnimation = () => {
  if (
    !ballStatus.value &&
    !dayHoverAnimationDelayId.value &&
    !shakeStatus.value
  ) {
...
const nightHoverAnimation = () => {
  ...
  if (
    !nightHoverAnimationStatus.value &&
    !nightHoverAnimationDelayId.value &&
    ballStatus.value
  ) {
    // 添加计时器
...
.button-box {
  ...
  .star-cloud-box {
    ...
    .star-box {
      ...
      transition: transform var(--move-duration)
        cubic-bezier(0.26, 0.97, 0.2, 1.08);
      ...
      .meteor {
        ...
        border-radius: 50%;
        transform: translate(
            calc(var(--box-height) * 1.75),
            calc(var(--box-height) * -0.35)
          )
          rotate(255deg);
    ...
    .cloud-box {
      ...
      .cloud-near {
        ...
        height: var(--box-height);
        width: var(--box-width);
        transition: transform var(--near-cloud-duration)
          cubic-bezier(0.49, 1.57, 0.04, 0.83);
        z-index: 2;
      ...
      .cloud-far {
        ...
        height: var(--box-height);
        width: var(--box-width);
        transition: transform var(--far-cloud-duration)
          cubic-bezier(0.49, 1.57, 0.28, 0.81);
        z-index: 1;
  ...
  .halo-box {
    ...
    .halo-linear {
      ...
      .halo-left {
        transform: translateX(
          calc(-50% + var(--ball-margin) + var(--ball-size) / 2)
        );
      }
      ...
      .halo-right {
        transform: translateX(
          calc(
            var(--box-width) - 50% - var(--ball-margin) - var(--ball-size) / 2
          )
        );
      }
    }
    ...
    .halo-flex {
      ...
      .halo-right {
        transform: translateX(
          calc(var(--box-width) - 100% + var(--ball-margin))
        );
      }
    }
  }
  ...
  .ball-box {
    ...
    .to-right {
      transform: translateX(
        calc(var(--box-width) - var(--ball-size) - var(--ball-margin))
      );
    }
...
@keyframes meteor-fall {
  ...
  40% {
    transform: translate(
        calc(var(--box-height) * -0.625),
        calc(var(--box-height) * 0.3)
      )
      rotate(255deg);
  }
